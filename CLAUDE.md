...
- ✅ 首页看板改版（2026-08-10）：①各所项目对比柱状图改「**运行中为主**」负载视图——柱高=各所当前运行中数，已完成/已终止作辅助数字显示在柱下（原三状态共轴，运行久后已完成累计值撑满 Y 轴、运行中/终止矮到不可见；改版后一眼看出「各所当前谁在忙」）②发起归档趋势增强——折线下**面积渐变** + **hover 十字线/tooltip**（显示发起/归档数值）+ **年度多数据点稀疏标注**（>12 点隔列标）。
