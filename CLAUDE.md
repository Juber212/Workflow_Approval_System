...
- ✅ 自动排产功能（2026-08-10）：项目/方案发起时按流程**工作节点**自动生成排产计划——新增 `schedule_items` 表 + `schedule_service`（自然日顺排、无工作日，从发起日按 `time_limit_days` 逐节点衔接；**资源冲突避免**：候选=节点人员[负责人>校验>审批>批准]，每人同时只能做一个，全忙则顺延到最早空闲者）；`GET /instances/{id}/schedule` API；实例详情页新增「排产计划」甘特图（`GanttChart.vue` 手绘 SVG，X 轴日期/Y 轴工序泳道）。329 测试全过（+3 排产单测）+ vue-tsc 0 错 + build 过。**扩展项**：跨所选人/按难度等级选人（规则未定）、排产重算（换人/时间变化）、甘特拖拽。
