...
- ✅ 文件模板包内模板只随包展示（2026-08-10，补修）：`getDocTemplates` 可关联列表排除**所有分类包内的模板**（含未关联分类）——包内模板只随分类包展示（linked/available_categories 的包里），「取消包关联」后模板留在「可关联分类」的包里、不再单独冒出来成为可关联单个；独立模板（不属于任何包）仍可单独关联。326 测试全过。

- ✅ 文件模板弹窗包内模板不单独显示（2026-08-10，补修前端）：`LaunchDocTemplateDialog.removeCategory` 移除分类后**不再把包内模板放回「可关联单个」列表**（此前本地放回导致「取消包关联后包内模板单独冒出来」，与后端 getDocTemplates 排除所有分类包内模板一致）。模板设计器与发起项目弹窗共用该组件，两处一并生效。vue-tsc 0 错 + build 过。
...
- ✅ 系统更名（2026-08-11）：全系统名称统一改为「**智展研究院设计开发管理系统**」（19 文件——前端浏览器标题/侧边栏品牌/登录页/全局样式注释、后端 APP_NAME/pyproject/依赖头注释、README/部署/用户手册文档；侧边栏长名改 12px 允许换行；历史审计文档保留原名）。326 测试全过 + vue-tsc 0 错 + build 过。

- ✅ 模块更名（2026-08-11）：「项目管理」模块改「**开工项目**」（侧边栏/路由 meta/页面标题/面包屑/使用与设计文档入口，业务词「项目管理周期」保留）；设计文档 00-03 系统名同步补全为「智展研究院设计开发管理系统」。vue-tsc 0 错 + build 过。

- ✅ 模板解耦（2026-08-11）：模板作为「快速发起项目」的工具，不与运行中项目耦合——发起项目弹窗（OrgHome）模板表格去掉「运行项目」列、模板详情（TemplateDetail）去掉「运行中项目」展示（组织维度运行中保留）。vue-tsc 0 错 + build 过。
...
- ✅ 桌面快捷方式分发指南（2026-08-12）：docs/桌面快捷方式分发指南.md——部署后给员工电脑批量创建系统桌面快捷方式（bat 一键脚本 + 内网共享分发 + 域 GPO 可选），访问地址用占位 `http://192.168.1.50` 待部署后替换。排产功能用户确认不做（方案文档已删）。
