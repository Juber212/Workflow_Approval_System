...
class Endorsement(Base):
    ...
    signature_x: Mapped[float | None] = mapped_column(Float, nullable=True, comment="批准人调整后的签名X坐标")
    signature_y: Mapped[float | None] = mapped_column(Float, nullable=True, comment="批准人调整后的签名Y坐标")
    signature_page: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="批准人选择的签名页码")
    decided_at: Mapped[datetime | None] = mapped_column(DateTime, comment="批准决定时间")
