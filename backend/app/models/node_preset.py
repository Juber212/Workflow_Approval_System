...
class NodePreset(Base):
    ...
    checkers: Mapped[list[dict] | None] = mapped_column(JSON, comment='校验人列表 [{"user_id": N}]')
    approvers: Mapped[list[dict] | None] = mapped_column(JSON, comment='审批人列表 [{"user_id": N}]')
    ...
    file_folders: Mapped[list[dict] | None] = mapped_column(JSON, comment="文件提交文件夹配置")
