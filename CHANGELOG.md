...
| # | 问题 | 涉及文件 |
|---|------|------|
| 1 | 进度条并行节点 SVG 分叉/汇合曲线偏上未居中 → `getBoundingClientRect` 统一坐标系 + SVG `marginTop` 对齐 viewBox 原点 + `offsetHeight/2` 精准圆心 | `ProgressBar.vue` |

### 新增

| # | 内容 | 涉及文件 |
|---|------|------|
| 2 | 测试数据种子脚本：22 实例覆盖全边界（4状态×4优先级×4难度×逾期/临期/驳回多轮/并行/跨组织/方案类型），幂等可重复运行 | `backend/app/core/seed_test_data.py`（新） |
